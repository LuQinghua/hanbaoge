package com.jiabian.util.performance;
    ...
    */
    ...
    public static final class Entry {
        ...
        private void toString(StringBuffer buffer, String prefix1, String prefix2) {
            ...
            StringBuffer pattern = new StringBuffer("起始时间{1,number}ms ");

            if (isReleased()) {
                pattern.append("[总持续{2,number}ms");

                if ((durationOfSelf > 0) && (durationOfSelf != duration)) {
                    pattern.append(" (自消耗{3,number}ms)");
                }

                if (percent > 0) {
                    pattern.append(", 占父比{4,number,##%}");
                }

                if (percentOfAll > 0) {
                    pattern.append(", 占总比{5,number,##%}");
                }
